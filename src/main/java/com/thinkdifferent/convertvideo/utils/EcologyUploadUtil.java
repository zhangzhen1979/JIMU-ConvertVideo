...
public class EcologyUploadUtil {
     ...
     * @return
     */
    public static JSONObject uploadFile2Ecology(String appId, String address, String api, String localFilePath, String ecologyFilePathId, String fileName) {
        if (StringUtils.isBlank(fileName)) {
            fileName = localFilePath.substring(localFilePath.lastIndexOf(File.separator));
        }
        // 地址添加 /
        if (StringUtils.isNotBlank(address) && !address.endsWith("/")) {
            address += "/";
        }
        // api 移除 /
        if (StringUtils.isNotBlank(api) && api.startsWith("/")) {
            api = api.substring(1);
        }
        //ECOLOGY返回的token
        ...
        if ("".equals(token) || null == token) {
            token = (String) getToken(address, appId).get("token");
        }
     ...
     */
    public static Map<String, Object> testRegist(String address, String appId) {
        //获取当前系统RSA加密的公钥
        RSA rsa = new RSA();
    ...
    public static Map<String, Object> getToken(String address, String appId) {
        return getToken(address, appId, 0);
    }
     ...
     */
    public static Map<String, Object> getToken(String address, String appId, int retryNum) {
        if (retryNum > 3) {
        ...
        if (StringUtils.equals("false", datas.get("status").toString())) {
            ...
            return getToken(address, appId, ++retryNum);
        }
